      ...
      return new Promise(resolve => {
        api.get('/v1/anjungan/cari-rencana-kontrol', params)
          .then(resp => {
            ...
            const rujukan = resp.data.result.flagKontrol === 'False' && resp.data?.result?.sep?.jnsPelayanan === 'Rawat Inap' && resp.data?.result?.sep?.provPerujuk?.kdProviderPerujuk === '1327R001' ? resp.data.result.sep.noSep : resp.data.result.sep.provPerujuk.noRujukan
