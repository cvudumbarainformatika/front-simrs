            ...
            <q-scroll-area style="height:calc(100% - 50px)">
              <q-list v-if="store?.items?.length" bordered separator>
                <q-expansion-item v-for="(item, i) in filteredItems" :key="i" expand-separator
                  :label="item?.ruangan + ' | ' + item?.rs1" :caption="tanggal(item?.tanggal)">
                  <q-card dark>
                    <q-separator />
                    ...
                    <q-list separator>
                      ...
                      <transition-group name="list">
                        <template v-for="(pemeriksaanfisik, f) in pasien?.pemeriksaanfisik" :key="f">
                          <!-- NADI-->
                        ...
                        <q-list dark separator>
                          <q-item v-for="(diagnosa, d) in item?.diagnosa" :key="d" v-ripple clickable>
                            <q-item-section>
                              ...
                              <q-item-label>Diagnosa = {{ diagnosa?.masterdiagnosa?.rs4 }} ( {{ diagnosa?.rs3 }}
                                )</q-item-label>
                              <q-item-label caption lines="2" />
